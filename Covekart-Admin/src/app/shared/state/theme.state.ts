...
export class ThemeState {
  ...
  @Action(UpdateTheme)
  updateTheme(ctx: StateContext<ThemesStateModel>, { id, status }: UpdateTheme) {
    return this.themeService.updateTheme(id, status).pipe(
      tap({
        next: (result) => {
          const state = ctx.getState();
          const updatedThemes = state.themes.data.map((theme) =>
            theme.id === id ? { ...theme, status } : theme
          );
          ...
          this.notificationService.showSuccess('Theme updated successfully!');
        },
        error: (err) => {
          this.notificationService.showError('Failed to update theme.');
          throw new Error(err?.error?.message);
        },
      })
    );
  ...
  @Action(UpdateHomePage)
  updateHomePage(
    ctx: StateContext<ThemesStateModel>,
    action: UpdateHomePage
  ) {
    return this.themeService.updateHomePage(action.id, action.payload).pipe(
      tap({
        ...
        error: (err) => {
          ...
          this.notificationService.showError('Failed to update the home page.');
          throw new Error(err?.error?.message);
        }
      })
    );
